...
public class ThongKeDTDAO {
    ...
    public ArrayList<ThongKeDT> getHoaDon(Date ngayBatDau, Date ngayKetThuc) {
        ArrayList<ThongKeDT> danhSachHoaDon = new ArrayList<>();
        ...
        try {
            ...
            while (rs.next()) {
                danhSachHoaDon.add(new ThongKeDT(rs.getString(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getDouble(5)));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<ThongKeDT> getDoanhThuThangNam(int month, int year) {
        ArrayList<ThongKeDT> danhSachHoaDon = new ArrayList<>();
        ConnectDB.getInstance();
        Connection con = ConnectDB.getConnection();
